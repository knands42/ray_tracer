...
private:
  ...
  double pixel_samples_scale =
      1.0;            // Color scale factor for a sum of pixel samples
  ...
  void initialize() {
    ...
    const auto viewport_upper_left =
        camera_center + viewport_center - viewport_u / 2 - viewport_v / 2;
  ...
  static auto ray_color(const Ray &ray, const int depth, const Hittable &world)
      -> Color {
    if (depth <= 0) {
      return {0, 0, 0};
    }

    if (HitRecord hitRecord;
        world.hit(ray, Interval(0.001, infinity), hitRecord)) {
      Ray scattered;
      Color attenuation;
